...
>("wallet/fetchDaiBalance", async ({ walletAddress, token }, thunkAPI) => {
...
>("wallet/fetchUsdcBalance", async ({ walletAddress, token }, thunkAPI) => {
  ...
  const balanceBig = await contract.balanceOf(walletAddress);
  const balanceInEther = ethers.utils.formatUnits(balanceBig, 6);
  return parseFloat(balanceInEther);
});

export const fetchDaiAllowance = createAsyncThunk<
  number,
...
export const fetchUsdcAllowance = createAsyncThunk<
  number,
  { walletAddress: string; address: string; token: Token }
>(
